...
**dsh-rtk 是一个薄层适配插件**，它把两个现有工具接在一起：

- **DSH（DeepSeek Harness）**：一个 AI 编程/agent 框架。LLM 通过"工具调用"（函数）执行 shell 命令，再把输出喂回给模型继续推理。输出越冗长，消耗的 token 越多。
- **RTK（Rust Token Killer）**：一个命令行工具，把 `git log`、`find`、`grep`、`read` 等命令的输出压缩成紧凑形式——"同样的信息，更少的 token"（实测单命令可省约 50%）。

**定位**：本插件**不 fork、不修改 RTK**。RTK 是底层能力提供者，插件只是把 RTK 接进 DSH 的接入层。所有命令变形/过滤逻辑都属于 RTK 本体。

## 为什么要有这个项目

RTK 只能在终端里手动敲，DSH 里的模型却只能调用工具、不能敲命令行；而且模型习惯用的 `pwsh` 工具会把命令的完整原文输出直接喂给模型——token 白白烧掉。

**dsh-rtk 就是那座桥**：让模型照常用 `pwsh(...)`，插件在背后把命令交给 RTK 执行，享受省 token 的红利，同时不破坏原有命令语义。
...
> 说明：插件**零运行时依赖**（只 import Node 内置模块）。`dsh plugin install` 本质是把参数转发给 profile 目录里的 pnpm，pnpm 原生支持 `github:` 依赖，克隆对应 tag 即可用——任何机器、任何用户都能这样装，不需要访问源码所在的机器。
...
```bash
cd <任一非系统 Temp 的工作目录>   # Windows ACL 沙箱要求 temp root 在 workspace 之外
...
- **v0.2.0**：agent 作用域遮蔽修复、服务注入重构、管道/PowerShell 开关保护、RTK_DB_PATH 沙箱回退。
